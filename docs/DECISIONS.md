...
- Persisted state version 3 adds `privateNotes` and custom-name synchronization flags. The reader supplies empty/default values when loading version-1/2 records, preserving existing sessions without destructive reset; the next successful write upgrades the serialized shape.
